import { canAccess } from './permissions.mjs';
...
export async function currentAdmin() {
  ...
  const { id, email, name, role, permissions, isActive } = session.user;
  return { id, email, name, role, permissions, isActive };
}
...
export async function authorize(request, screen) {
  ...
  if (screen && !canAccess(user, screen)) return Response.json({ error: 'Bạn không có quyền sử dụng chức năng này.' }, { status: 403 });
  return null;
}
